    ...
    article["ai_artifacts"] = [
        {
            ...
            "output_json": json.dumps(
                {
                    "summary": "LEGACY_ONLY <script>alert(1)</script> 中文摘要",
                    "key_points": ["安全重点"],
                    "language": "zh-CN",
                    "basis": "metadata",
                    "limitations": "",
        ...
        },
        {
            ...
            "output_json": json.dumps(
                {
                    "title": "发布方标题译文",
                    "publisher_summary": "<script>alert(2)</script> 发布方简介译文",
                    "language": "zh-CN",
                    "limitations": "",
                },
                ensure_ascii=False,
...
class AIRenderTests(unittest.TestCase):
    ...
    def test_latest_json_keeps_ai_in_an_independent_field(self):
        ...
        self.assertEqual(1, len(public_article["ai_artifacts"]))
        self.assertEqual("translation", public_article["ai_artifacts"][0]["task"])
        self.assertNotIn("provider", public_article["ai_artifacts"][0])
        self.assertNotIn("model", public_article["ai_artifacts"][0])
        self.assertEqual(1, payload["cached_ai_artifact_count"])
