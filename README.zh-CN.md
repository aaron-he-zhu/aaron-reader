...
Aaron Reader 是一个订阅 OpenAI 与 Anthropic 官方内容的双语云端阅读器。GitHub Actions 负责收集、校验新文章；默认由 OpenRouter Free 将每篇文章的标题和发布方摘要翻译为简体中文并生成日／周简报，DeepSeek V4 Flash 提供有界兜底；Cloudflare Workers 提供最终的只读网页。
...
英文是默认界面；语言选择器提供完整的简体中文界面，并在存在中文缓存时自动显示译文。网页没有客户端“翻译”按钮，也不生成独立的单篇 AI 摘要。
...
任何结果不明的情况都不会自动兜底，包括超时、连接中断、`408`／`409`／`425`、`5xx`、损坏、无类型、信号冲突或截断的响应、未知用量、已有 ambiguous／paid-failure 生成冻结、预算不足、除前述“发送前缺少 OpenRouter 凭据”之外的其他本地配置或输入错误，以及 safety、moderation、content filter 或 policy 拒答。`400`、`403`、`422` 等明确但不可兜底的请求或政策失败会建立跨 profile 的 paid-failure 冻结。`fallback_pending` 是唯一窄例外：它只授权预先配置的 DeepSeek 续跑，绝不授权主提供方重放。模糊结果或不可兜底的已计费失败会建立跨提供方语义冻结，更换 provider 也不能绕过；如需重放，必须显式选择提供方，并用 `force_held` 确认接受潜在重复调用。

每次真正发出 provider POST 前，本地 attempt 状态与 `ambiguous` 防重放冻结会在同一个 SQLite 事务中提交；明确响应会原子结算这份临时冻结。由 provider 生成的报告 artifact、公开 report 索引、用量与 attempt 完成状态也在同一事务中提交。只要工作流随后成功导出并发布 `cloud/ai-cache.json`，这些保护就能覆盖 Python／CLI 进程失败。但如果整台托管 runner 在请求到达提供方后、尚未导出本地数据库前彻底丢失，这并不是绝对 exactly-once 保证；系统也不假定提供方支持服务端幂等。生产工作流串行化为单写者，临时冻结的结算逻辑依赖这一边界。
...
- `cloud/ai-cache.json` 保存经过校验的翻译、为兼容旧版本而保留的历史单篇摘要，以及日／周报，并以稳定的发布方身份与内容哈希为键，而不是使用临时数据库行 ID。新的生产任务不再生成单篇摘要，公开阅读器投影也不会发布这些历史摘要。缓存还保存有界的旧金山时区聚合用量账本，以及不含输入内容的生成冻结指纹，让预算与防重放保护能够跨一次性 runner 延续。它不包含 API key、提供方请求 ID、逐请求审计、错误正文、prompt、模型响应、个人阅读状态或提取后的完整文章正文。
...
正式提交前，工作流会把刚导出的两份交接文件再次导入第二个空白验证数据库；网站只能从这个重建数据库渲染。如果网页中的报告身份与公开 AI 缓存不同，发布会直接失败。因此，只靠仓库里的公开状态就能在另一台一次性 runner 上重现部署站点。
...
- 结果不明、用量未知或安全／政策拒答绝不会兜底，并会建立适当的稳定生成冻结，防止后续定时任务静默重放。
...
打开仓库的 **Actions** 页面，在 `.github/workflows/` 下选择生产更新工作流，点击 **Run workflow**，保留默认的 `openrouter` 主提供方，或选择 `deepseek` 执行仅 DeepSeek 的诊断任务，然后在 `main` 上运行。手动任务与定时任务使用完全相同的临时数据库、Secret 边界、缓存规则、安全上限、测试、精确文件提交和 Cloudflare 发布路径。`force_weekly` 是一次性的强制周报开关；`force_held` 是另一项恢复开关，它可能重复一笔已经计费的生成。除非已经查看工作流 Summary 并明确接受这笔费用，否则不要启用 `force_held`。定时任务不会启用这两个开关。
