...
Aaron Reader 是一个订阅 OpenAI 与 Anthropic 官方内容的双语云端阅读器。GitHub Actions 负责收集、校验新文章；默认由 OpenRouter Free 将每篇文章的标题和发布方摘要翻译为简体中文，DeepSeek V4 Flash 提供有界兜底；Cloudflare Workers 提供最终的只读网页。
...
打开仓库的 **Actions** 页面，在 `.github/workflows/` 下选择生产更新工作流，点击 **Run workflow**，保留默认的 `openrouter` 主提供方，或选择 `deepseek` 执行仅 DeepSeek 的诊断任务，然后在 `main` 上运行。手动任务与定时任务使用完全相同的临时数据库、Secret 边界、缓存规则、安全上限、测试、精确文件提交和 Cloudflare 发布路径。`force_held` 是一项恢复开关，它可能重复一笔已经计费的生成。除非已经查看工作流 Summary 并明确接受这笔费用，否则不要启用 `force_held`。定时任务不会启用这项开关。
