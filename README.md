...
Ambiguous results never trigger fallback or scheduled replay. Timeouts, connection failures, `408`/`409`/`425`, `5xx`, malformed, untyped, conflicting, or truncated provider responses, unknown usage, pre-existing generation holds, budget failures, all other local configuration/input errors, and safety, moderation, content-filter, or policy refusals do not themselves authorize cross-provider fallback. The sole configuration exception is the missing pre-send OpenRouter credential described above. Definite non-fallback request/policy failures such as `400`, `403`, and `422` create a cross-profile paid-failure hold. A `fallback_pending` hold is the sole narrow hold exception for cross-provider continuation: it authorizes only the configured DeepSeek continuation and never a replay on the primary provider.

Scheduled backfill has a separate, narrower paid-failure replay rule. For one missing article, the schedule may authorize at most one replay on the currently active provider profile in that cycle, and only when every semantically equivalent hold is `paid_failure`. Any equivalent `ambiguous` hold blocks automatic replay. The replay consumes the normal daily request/token budget and may still receive the existing single OpenRouter-to-DeepSeek continuation when a new OpenRouter attempt meets the closed fallback rules; it never creates a third call. A repeatedly failing paid item can therefore create one new billable replay in each later scheduled cycle until it succeeds, becomes ambiguous, or reaches the budget gate. If an earlier article has already switched the cycle to DeepSeek, later articles use that active profile without reversing back to OpenRouter. Manual workflow runs do not enable this all-paid-failure policy by default. `force_held` remains the explicit broad recovery control and can bypass ambiguous as well as paid-failure holds, so it requires the operator's acknowledgement of possible duplicate billing. Merely changing provider never bypasses a hold.
...
- each scheduled cycle grants at most one initial provider attempt or narrowly authorized paid-failure replay per missing article, translating only its title and publisher summary;
